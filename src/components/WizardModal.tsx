...
const WizardModal = () => {
  ...
  return <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
      <DialogContent className={`${currentStep === 6 ? 'max-w-full w-screen sm:w-[95vw] h-screen sm:h-[95vh]' : 'w-screen sm:max-w-[800px] h-screen sm:h-[500px]'} p-0 gap-0 bg-background border-0 overflow-hidden`} style={{ zIndex: 10000 }}>
        ...
        <button onClick={handleClose} className="absolute right-4 top-4 rounded-sm bg-wizard-sidebar text-white p-1.5 transition-opacity hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2" style={{ zIndex: 10001 }}>
          <X className="h-5 w-5" />
